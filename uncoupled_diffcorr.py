...
#%%
def get_POGuessLinear_uncoupled(eqNum,Ax,par):
...
#
    ...
    eqPos = get_eq_pts_uncoupled(eqNum, par);
    ...
    def eqPointEig_uncoupled(ep, parameters):
        ...
        Df = jacobian_uncoupled(ep, parameters);
    ...
    [Es,Eu,Ec,Vs,Vu,Vc] = eqPointEig_uncoupled(eqPt, par)
...
#%%
def half_period(t,x):
    # Return the value that we want to be zero
    #
    #       t = 0                    t = T/2                             t = T
...
def get_PODiffCorr_POFam(x0, par):
    ...
    while abs(dxdot1) > MAXdxdot1:
        ...
        f = partial(uncoupled2dof, par=par)  # Use partial in order to pass parameters to function
        ...
        t,x,phi_t1,PHI = stateTransitMat_uncoupled(t1,x0,par) ;
...
#%%
def get_PODiffCorr_uncoupled(x0, par):
...
#%%
def get_POFam_uncoupled(eqNum,Ax1,Ax2,nFam,po_fam_file,par):
    ...
    x0poGuess1,TGuess1 = get_POGuessLinear_uncoupled(eqNum,Ax1,par);
    x0poGuess2,TGuess2 = get_POGuessLinear_uncoupled(eqNum,Ax2,par);
    ...
    energyPO[iFam] = get_total_energy_uncoupled(x0po1, par) ;
    ...
    energyPO[iFam] = get_total_energy_uncoupled(x0po2, par) ;
...
#         T   [iFam]     = 2*t1_iFam ;	
        ...
        energyPO[i] = get_total_energy_uncoupled(x0po[i,:], par) ;
...
#%%
def poBracketEnergy_uncoupled(energyTarget,x0podata, po_brac_file, par):
    ...
    T[0]        = x0podata[-2,N];
    T[1]        = x0podata[-1,N];
    energyPO[0] = get_total_energy_uncoupled(x0po[0,0:N],par)
    energyPO[1] = get_total_energy_uncoupled(x0po[1,0:N],par)
    ...
    #                 (x0po(iFam-1,2)), 0, 0] ;
            ...
            [x0po_iFam,tfpo_iFam] = get_PODiffCorr_uncoupled(x0po_g, par) ; 
            energyPO[iFam] = get_total_energy_uncoupled(x0po_iFam, par);
            x0po[iFam,:] = x0po_iFam ;
            T[iFam]      = 2*tfpo_iFam ;
            iFam = iFam + 1;
    ...
    #                 (x0po(iFam-2,2)) 0 0] ;
            ...
            [x0po_iFam,tfpo_iFam] = get_PODiffCorr_uncoupled(x0po_g, par) ; 
            energyPO[iFam-1] = get_total_energy_uncoupled(x0po_iFam, par);
...
#%%
def poTargetEnergy_uncoupled(x0po, energyTarget, po_target_file,par):
    #
    ...
    #         c = [a(1) + dx a(2) + dy 0 0];
        ...
        [x0po_iFam,tfpo_iFam] = get_PODiffCorr_uncoupled(c, par)
        
        energyPO = get_total_energy_uncoupled(x0po_iFam, par)
    ...
    #         T(iFam,1)      = 2*tfpo_iFam ;
        ...
        if (abs(get_total_energy_uncoupled(c, par) - energyTarget) < energyTol) or (iter == iterMax):
            print('Initial condition: %s\n' %c);
            print('Energy of the initial condition for PO %s\n' %get_total_energy_uncoupled(c, par) );
            x0_PO = c
            T_PO = 2*tfpo_iFam; 
            ePO = get_total_energy_uncoupled(c, par);
            break
        
        
        if np.sign( get_total_energy_uncoupled(c, par) - energyTarget ) == np.sign ( get_total_energy_uncoupled(a, par) - energyTarget ):
            a = c
        else:
            b = c;
    ...
    print('Iterations completed: %s, error in energy: %s \n' %(iter, abs(get_total_energy_uncoupled(c, par) - energyTarget)));
