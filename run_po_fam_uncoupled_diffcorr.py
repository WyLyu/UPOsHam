...
#"""   SCRIPT to compute periodic orbits for the 2 DoF Uncoupled potential
#--------------------------------------------------------------------------
#   uncoupled potential energy surface notations:
#  H = 1/2*px^2+omega/2*py^2 -(1/2)*alpha*x^2+1/4*beta*x^4+ omega/2y^2
#           Well (stable, EQNUM = 2)    
#
#               Saddle (EQNUM=1)
#
...
# model 
po_target_file = open("1111x0po_T_energyPO_eqPt%s_DelE%s_uncoupled.txt" %(eqNum,deltaE), 'a+')
                
[x0_PO, T_PO, e_PO] = uncoupled_diffcorr.poTargetEnergy_uncoupled(x0poTarget,eTarget,po_target_file,parameters);
