...
#%%
def dotproduct(model,guess1, guess2,n_turn,par):
    """
    ...
    RelTol = 3.e-10
    AbsTol = 1.e-10 
    f1 = lambda t,x: Ham2dof(model,t,x,par) 
    soln1 = solve_ivp(f1, TSPAN, guess1,method='RK45',dense_output=True, events = lambda t,x: half_period(t,x,model),rtol=RelTol, atol=AbsTol)
    te1 = soln1.t_events[0]
    t1 = [0,te1[n_turn]]#[0,te1[1]]
    turn1 = soln1.sol(t1)
    x_turn1 = turn1[0,-1] 
    y_turn1 = turn1[1,-1] 
    t,xx1,phi_t1,PHI = stateTransitMat(t1,guess1,par,model)
    x1 = xx1[:,0]
    y1 = xx1[:,1]
    p1 = xx1[:,2:]
    p_perpendicular_1 = math.sqrt(np.dot(p1[-3,:],p1[-3,:]))*p1[-2,:] - np.dot(p1[-2,:],p1[-3,:])*p1[-3,:]
    f2 = lambda t,x: Ham2dof(model,t,x,par)  
    soln2 = solve_ivp(f2, TSPAN, guess2,method='RK45',dense_output=True, events = lambda t,x: half_period(t,x,model),rtol=RelTol, atol=AbsTol)
    te2 = soln2.t_events[0]
    t2 = [0,te2[n_turn]]#[0,te2[1]]
    turn2 = soln1.sol(t2)
    x_turn2 = turn2[0,-1] 
    y_turn2 = turn2[1,-1] 
    t,xx2,phi_t1,PHI = stateTransitMat(t2,guess2,par,model)
    x2 = xx2[:,0]
    y2 = xx2[:,1]
    p2 = xx2[:,2:]
...
#%%
def turningPoint(model,begin1,begin2,par,e,n,n_turn,po_fam_file):
    """
    ...
    while iter < MAXiter and n_turn < 10:
        ...
        for i in range(0,n+1):
            ...
            x_turn1,x_turn2,y_turn1,y_turn2, dotpro = dotproduct(model,guess1, guess,n_turn,par)
            result[i,0] = dotpro
            result[i,1] = guess[0]
            result[i,2] = guess[1]
        ...
        if check_same == i_turn[iter] and check_diff == -n+i_turn[iter]-1 and i_turn_iter>0:
            ...
            RelTol = 3.e-10
            AbsTol = 1.e-10
            f = lambda t,x: Ham2dof(model,t,x,par) 
            soln = solve_ivp(f, TSPAN, guesspo,method='RK45',dense_output=True, events = lambda t,x: half_period(t,x,model),rtol=RelTol, atol=AbsTol)
            te = soln.t_events[0]
            tt = [0,te[1]]
            t,x,phi_t1,PHI = stateTransitMat(tt,guesspo,par,model)
            T[iter] = tt[-1]*2
            print("period is%s " %T[iter])
            energy = np.zeros(len(x))
            #print(len(t))
            for j in range(len(t)):
                energy[j] = get_total_energy(model,x[j,:],par)
...
#%%
def configdiff(model,guess1, guess2,n_turn,par):
    """
    configdiff(model,guess1, guess2,n_turn,par) returns the difference of x(or y) coordinates between the guess initial conditions and the ith turning points
    n_turn is the nth turning point we want to choose as our 'turning point' for defining the dot product
    ...
    if model == 'uncoupled':
        ...
        print("Initial guess1%s, intial guess2%s, x_diff1 is %s, x_diff2 is%s, y_diff1 is %s, y_diff2 is%s " %(guess1,guess2,x_diff1, x_diff2,y_diff1, y_diff2))
        return x_diff1, x_diff2,y_diff1, y_diff2


#%%
def turningPoint_configdiff(model,begin1,begin2,par,e,n,n_turn,po_fam_file):
    """
    turningPoint(model,begin1,begin2,par,e,n,n_turn,po_fam_file) finds the initial condition of the PO with total energy e, given 2 initial guesses guess1 and guess2 using the turning point based on configuration difference method.
    n is the number of divisons we want to divide
    n_turn is the nth turning point we want to choose as our 'turning point for defining the dot product'
    e is the energy of the PES 
    po_fam_file is the file we want to save our data into 
    ...
    while iter < MAXiter and n_turn < 5:
        ...
        for i in range(0,n+1):
            ...
            if model == 'uncoupled':
                ...
                break
            guess = [xguess,yguess,0, 0]
            coordinate_diff1, coordinate_diff2 = configdiff(model,guess1, guess,n_turn,par)
