"""
Created on Wed Jul 10 19:15:22 2019
...
#%%
def get_POGuessLinear_deleonberne(eqNum,Ax,par):
...
#
    ...
    def eqPointEig_deleonberne(ep, parameters):
...
def half_period(t,x):
    ...
    # In the deleonberne problem we use the 3rd equation.
    #
    #       t = 0                    t = T/2                             t = T
...
def get_PODiffCorr_POFam(x0, par):
    ...
    while abs(dydot1) > MAXdydot1:
        ...
        f = partial(deleonberne2dof, par=par)  # Use partial in order to pass parameters to function
        soln1 = solve_ivp(f, TSPAN, x0,method='RK45',dense_output=True, events = half_period,rtol=RelTol, atol=AbsTol)
        te = soln1.t_events[0]  # the first element of te is the initial point
        t1 = [0,te[1]]
        xx1 = soln1.sol(t1)
        x1 = xx1[0,-1] 
        y1 = xx1[1,-1] 
        ...
        t,x,phi_t1,PHI = stateTransitMat_deleonberne(t1,x0,par) ;
        ...
        if show == 1:
            ...
            for i in range(0,len(x)):
                ...
                e[i] = get_total_energy_deleonberne(x[i,:],par);
        ...
        dVdx = -2*par[3]*par[4]*math.e**(-par[4]*x1)*(math.e**(-par[4]*x1) - 1) - 4*par[5]*par[4]*y1**2*(y1**2 - 1)*math.e**(-par[5]*par[4]*y1)
...
#       correctx0 = dxdot1/(phi_t1[3,1] - phi_t1[4,1]*(vxdot1/vydot1));	
#       x0[0] = x0[0] - correctx0;
...
#%%
def get_PODiffCorr_deleonberne(x0, par):
    ...
    # tolerances for integration and perpendicular crossing of x-axis
    # MAXdydot1 = 1.e-8 ;RelTol = 3.e-10; AbsTol = 1.e-10; 
...
#%%
def get_POFam_deleonberne(eqNum,Ax1,Ax2,nFam,po_fam_file,par):
    ...
    x0poGuess1,TGuess1 = get_POGuessLinear_deleonberne(eqNum,Ax1,par);
    x0poGuess2,TGuess2 = get_POGuessLinear_deleonberne(eqNum,Ax2,par);
    ...
    energyPO[iFam] = get_total_energy_deleonberne(x0po1, par) ;
    ...
    energyPO[iFam] = get_total_energy_deleonberne(x0po2, par) ;
...
#         T   [iFam]     = 2*t1_iFam ;	
        ...
        energyPO[i] = get_total_energy_deleonberne(x0po[i,:], par) ;
...
def poBracketEnergy_deleonberne(energyTarget,x0podata, po_brac_file, par):
    ...
    T[0]        = x0podata[-2,N];
    T[1]        = x0podata[-1,N];
    energyPO[0] = get_total_energy_deleonberne(x0po[0,0:N],par)
    energyPO[1] = get_total_energy_deleonberne(x0po[1,0:N],par)
    ...
    #                 (x0po(iFam-1,2)), 0, 0] ;
            ...
            energyPO[iFam] = get_total_energy_deleonberne(x0po_iFam, par);
            x0po[iFam,:] = x0po_iFam ;
            T[iFam]      = 2*tfpo_iFam ;
            iFam = iFam + 1;
...
#%%
def poTargetEnergy_deleonberne(x0po, energyTarget, po_target_file,par):
    #
    ...
    #         T(iFam,1)      = 2*tfpo_iFam ;
        ...
        if (abs(get_total_energy_deleonberne(c, par) - energyTarget) < energyTol) or (iter == iterMax):
        ...
        if np.sign( get_total_energy_deleonberne(c, par) - energyTarget ) == np.sign ( get_total_energy_deleonberne(a, par) - energyTarget ):
            a = c
        else:
            b = c;
