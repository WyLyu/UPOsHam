...
eqNum = 1;  
eqPt = coupled_newmethod.get_eq_pts_coupled(eqNum, parameters)
...
state0_2 = [0.0,coupled_newmethod.get_y(0.0,e,parameters),0.0,0.0]
state0_3 = [0.05, coupled_newmethod.get_y(0.05,e,parameters),0.0,0.0]


po_fam_file = open("1111x0_newmethod_deltaE%s_coupled.txt" %(deltaE),'a+')
[x0po_1, T_1,energyPO_1] = coupled_newmethod.newmethod_coupled(state0_2,state0_3 ,parameters,e,n,n_turn,po_fam_file) ; 
po_fam_file.close()
...
deltaE = e-parameters[2]
"""Trial initial Condition s.t. one initial condition is on the LHS of the UPO and the other one is on the RHS of the UPO"""
state0_2 = [-0.05,coupled_newmethod.get_y(-0.05,e,parameters),0.0,0.0]
state0_3 = [0.10, coupled_newmethod.get_y(0.10,e,parameters),0.0,0.0]


po_fam_file = open("1111x0_newmethod_deltaE%s_coupled.txt" %(deltaE),'a+')
[x0po_2, T_2,energyPO_2] = coupled_newmethod.newmethod_coupled(state0_2,state0_3 ,parameters,e,n,n_turn,po_fam_file) ; 
po_fam_file.close()
...
state0_3 = [0.18, coupled_newmethod.get_y(0.18,e,parameters),0.0,0.0]


po_fam_file = open("1111x0_newmethod_deltaE%s_coupled.txt" %(deltaE),'a+')
[x0po_3, T_3,energyPO_3] = coupled_newmethod.newmethod_coupled(state0_2,state0_3 ,parameters,e,n,n_turn,po_fam_file) ; 
po_fam_file.close()
...
po_fam_file = open("1111x0_newmethod_deltaE%s_coupled.txt" %(deltaE),'a+')
[x0po_4, T_4,energyPO_4] = coupled_newmethod.newmethod_coupled(state0_2,state0_3 ,parameters,e,n,n_turn,po_fam_file) ; 
po_fam_file.close()
...
po_fam_file = open("1111x0_newmethod_deltaE%s_coupled.txt" %(deltaE),'a+')
[x0po_5, T_5,energyPO_5] = coupled_newmethod.newmethod_coupled(state0_2,state0_3 ,parameters,e,n,n_turn,po_fam_file) ; 
po_fam_file.close()
...
RelTol = 3.e-10
AbsTol = 1.e-10
f = partial(coupled_newmethod.coupled2dof, par=parameters) 
soln = solve_ivp(f, TSPAN, x0po_1,method='RK45',dense_output=True, events = coupled_newmethod.half_period,rtol=RelTol, atol=AbsTol)
te = soln.t_events[0]
tt = [0,te[2]]
...
f = partial(coupled_newmethod.coupled2dof, par=parameters) 
soln = solve_ivp(f, TSPAN, x0po_2,method='RK45',dense_output=True, events = coupled_newmethod.half_period,rtol=RelTol, atol=AbsTol)
te = soln.t_events[0]
tt = [0,te[2]]
...
f = partial(coupled_newmethod.coupled2dof, par=parameters) 
soln = solve_ivp(f, TSPAN, x0po_3,method='RK45',dense_output=True, events = coupled_newmethod.half_period,rtol=RelTol, atol=AbsTol)
te = soln.t_events[0]
tt = [0,te[2]]
...
f = partial(coupled_newmethod.coupled2dof, par=parameters) 
soln = solve_ivp(f, TSPAN, x0po_4,method='RK45',dense_output=True, events = coupled_newmethod.half_period,rtol=RelTol, atol=AbsTol)
te = soln.t_events[0]
tt = [0,te[2]]
...
f = partial(coupled_newmethod.coupled2dof, par=parameters) 
soln = solve_ivp(f, TSPAN, x0po_5,method='RK45',dense_output=True, events = coupled_newmethod.half_period,rtol=RelTol, atol=AbsTol)
te = soln.t_events[0]
tt = [0,te[2]]
...
cset1 = ax.contour(xMat, yMat, coupled_newmethod.get_pot_surf_proj(xVec, yVec,parameters), [0.01,0.1,1,2,4],zdir='z', offset=0,
