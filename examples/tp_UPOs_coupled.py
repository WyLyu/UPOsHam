...
eqPt = turning_point.get_eq_pts(eqNum, init_guess_eqpt_coupled, \
...
#x=-0.05
#f2 = lambda y: turning_point.get_coordinate(model,0.0,y,e,parameters)
...
#
#x=0.05
#f3 = lambda y: turning_point.get_coordinate(model,x,y,e,parameters)
#yanalytic = math.sqrt(2/(parameters[1]+parameters[6]))*(-math.sqrt( e +0.5*parameters[3]* x**2- 0.25*parameters[4]*x**4 -0.5*parameters[6]* x**2 + (parameters[6]*x)**2/(2*(parameters[1] +parameters[6]) )) +parameters[6]/(math.sqrt(2*(parameters[1]+parameters[6])) )*x ) #coupled
#state0_3 = [x, optimize.newton(f3,yanalytic),0.0,0.0]
#
#
...
for i in range(len(deltaE_vals)):
    ...
    te = soln.t_events[0]
    tt = [0,te[2]]
    t,x,phi_t1,PHI = turning_point.stateTransitMat(tt, x0po[:,i], parameters, varEqns_coupled)
...
#yVec = np.linspace(-4,4,resX)
#xMat, yMat = np.meshgrid(xVec, yVec)
#cset1 = ax.contour(xMat, yMat, turning_point.get_pot_surf_proj(model,xVec, yVec,parameters), [0.01,0.1,1,2,4],zdir='z', offset=0,
