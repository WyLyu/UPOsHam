...
po_fam_file = open("x0_diffcorr_fam_eqPt%s_uncoupled.dat" %eqNum,'a+')
...
for i in range(len(deltaE_vals)):
    ...
    #%
    po_brac_file = open("x0po_T_energyPO_eqPt%s_brac%s_uncoupled.dat" %(eqNum,deltaE),'a+')
    t = time.time()
    ...
    model_parameters_file = open("model_parameters_eqPt%s_DelE%s_uncoupled.dat" %(eqNum,deltaE),'a+')
    ...
    po_target_file = open("x0_diffcorr_deltaE%s_uncoupled.dat" %(deltaE),'a+')
...
for i in range(len(deltaE_vals)):
    ...
    po_fam_file = open("x0_diffcorr_deltaE%s_uncoupled.dat" %(deltaE),'a+')
    print('Loading the periodic orbit family from data file',po_fam_file.name,'\n') 
    x0podata = np.loadtxt(po_fam_file.name)
    po_fam_file.close()
    x0po[:,i] = x0podata[0:4]
