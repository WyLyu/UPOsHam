...
t = time.time()
# [x0poTarget,TTarget] = bracket_POEnergy_bp(eTarget, x0podata, po_brac_file)
x0poTarget,TTarget = diffcorr_UPOsHam2dof.poBracketEnergy_uncoupled(eTarget, x0podata,po_brac_file, parameters)
poTarE_runtime = time.time()-t
model_parameters_file = open("1111model_parameters_eqPt%s_DelE%s_uncoupled.txt" %(eqNum,deltaE),'a+')
np.savetxt(model_parameters_file.name, parameters,fmt='%1.16e')
model_parameters_file.close()
...
[x0po_2, T_2,energyPO_2] = diffcorr_UPOsHam2dof.poTargetEnergy_uncoupled(x0poTarget,eTarget,po_target_file,parameters)
...
cset2 = ax.contour(xMat, yMat, diffcorr_UPOsHam2dof.get_pot_surf_proj(model,xVec, yVec,parameters), [0.01,0.1,1,2,4],zdir='z', offset=0,
                       linewidths = 1.0, cmap=cm.viridis, alpha = 0.8)
